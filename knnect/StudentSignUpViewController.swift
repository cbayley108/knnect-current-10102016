...
class StudentSignUpViewController: UITableViewController, TagListViewDelegate {
    ...
    var timer: NSTimer?
    var defaultTexts = ["first-name": "", "last-name": "", "school": "", "grade": "", "interests": "", "bio": "", "headline": ""]
    var interestsArray = [""]
    ...
    func setDefaultTexts(){
        ...
        gradeField.text = self.defaultTexts["grade"]
    ...
    @IBAction func editSavePressed(sender: AnyObject) {
        ...
        let fieldsArray = [ firstNameField, lastNameField, schoolField, gradeField, headlineField ]
        ...
        if(firstNameField.text == defaultTexts["first-name"] &&
            ...
            gradeField.text == defaultTexts["corporation"] &&
        ...
        if fieldsCompleted{
            //Update firebase if any of the fields were changed
            if fieldsChanged {
                ...
                self.userRef!.child("grade").setValue(gradeField.text)
                ...
                self.userRef!.child("type").setValue("Student")
            }
    ...
    @IBAction func savePressed(sender: AnyObject) {
        //Checks if all fields have been completed
        let fieldsArray = [ firstNameField, lastNameField, schoolField, gradeField]
        var fieldsCompleted = true
        for item in fieldsArray{
            fieldsCompleted = fieldsCompleted && (item.hasText())
        }
        fieldsCompleted = fieldsCompleted && (bioField.hasText()) && userAgreementSwitch.on && selectedInterests.count > 0
        //Alert for displaying license agreements
